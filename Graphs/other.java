...
public class other {
    ...
    //Articultion point  Tc O(V+E)
    public static void dffs(ArrayList<Edge> graph[],int curr,int par,int dt[],int low[],int time,boolean vis[],boolean ap[]){
        vis[curr] = true;
        dt[curr] = low[curr] = ++time;
        int children = 0;
        ...
        for(int i=0;i<graph[curr].size();i++){
            ...
            if(par == neigh){
                ...
                dffs(graph, neigh, curr,dt,low,time,vis,ap);
                low[curr] = Math.min(low[curr], low[neigh]);
    ...
    public static void getAp(ArrayList<Edge> graph[],int V){
        ...
        int time = 0;
        boolean vis[] = new boolean[V];
        //sometimes for large it may give same ap so
        boolean ap[] = new boolean[V];//if ap there then T
        
        for(int i=0;i<V;i++){
            if(!vis[i]){
                //dfs()
                dffs(graph, i, -1, dt, low, time, vis,ap);
            }
        }
        //print all AP
        for(int i=0;i<V;i++){
